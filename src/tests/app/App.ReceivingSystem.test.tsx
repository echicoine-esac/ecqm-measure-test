...
test(thisTestFile + ' renders properly', async () => {
  ...
  //'Selected Measure'
  expect(screen.getByTestId('rec-sys-server-dropdown')).toBeInTheDocument();
  expect(screen.getByTestId('rec-sys-submit-button')).toBeInTheDocument();
  expect(screen.getByTestId('rec-sys-hide-section-button')).toBeInTheDocument();

});


test(thisTestFile + ' fail scenario: submit without any selection', async () => {
  await act(async () => {
    render(<App />);
  });
...
test(thisTestFile + ' success scenario: submit', async () => {
...
test(thisTestFile + ' fail scenario: submit without server selection', async () => {
  const dataServers: Server[] = Constants.serverTestData;
  const mockMeasureList: Measure[] = await buildMeasureData(dataServers[0].baseUrl);
  const mockPatientList: Patient[] = await buildPatientData(dataServers[0].baseUrl);
